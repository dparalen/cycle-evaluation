...
Brent's\cite{wikipedia:loop} has been adopted here.
...
The cycle effectivelly blocking the rest of the inspected trace renders
the obviously true property invalid; see the
figure \ref{spiral}.
\begin{figure}
%\input{spiral}
...
%		\BEGIN
			...
			|Initialize numerics|(p_0, k_{\omega});
			|Initialize|\sim(\varphi, p_0);
			k, l := \keyword{None}, \keyword{None};
			p:=p_0;
			\tilde{\pi}:=p;
			\WHILE |True|
			\DO
				\WHILE p\sim|numerics|(p)
				\DO
					p:=|numerics|(p);
				\OD
				\IF (k,l):=|cycle|(p)
				\THEN
					\keyword{break};
				\FI
				\IF |end| |of| |numerics|(p)
				\THEN
					\keyword{break};
				\FI
				\tilde{\pi}:=\tilde{\pi}\cdot p
			\OD
...
\begin{algorithm}
	...
	\begin{program}
		...
		\BEGIN
			...
			\IF l \neq \keyword{None}
			\THEN
				\FOREACH a\in {}_l[[\varphi]]_k^0 \rcomment{cyclic}
				\DO
					|result| := |result| \wedge a(\tilde{\pi});
				\OD
			\ELSE
				\FOREACH a\in [[\varphi]]_k^0 \rcomment{acyclic}
				\DO
					|result| := |result| \wedge a(\tilde{\pi});
				\OD
			\FI
