	...
	\begin{algorithmic}[1]
		...
		\WHILE {$steps < k_{\omega}$}
			\WHILE {$p\sim$ numerical simulation in $p \wedge steps < k_{\omega}$}
				...
				\IF {numerical precission lost in $p$}
					\STATE $l\leftarrow k$ \COMMENT{selfloop--equilibrium}
					\RETURN $\tilde{\pi},k,l$
				\ENDIF
			...
			\STATE $k\leftarrow k+1$ \COMMENT{new representant}
		\ENDWHILE
		\RETURN $\tilde{\pi},k,l$
	\end{algorithmic}
\end{algorithm}
...
result. The input parameters are again $\varphi,p_0,k_\omega$.
...
The Byramov\cite{bayramov} \textsc{ode} system as used in Dra\v{z}an's
evaluation is utilized here, too. This time, the $216$ seeds are located
in a~rectangular \textsc{3d} grid in
...
$9600$ steps from the range $\langle 0.0,960\rangle$.
\begin{table}[h]
\label{tab:by}
\begin{tabular}{l | c r}
...
Due to accuracy of the integrator, 8 000 numerical method
...
\item experiments have got hard-coded setting;
...
Since the are no performance measures taken from the experiments, 
